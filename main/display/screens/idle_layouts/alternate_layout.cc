#include "alternate_layout.h"
#include <esp_log.h>

#define TAG "AlternateLayout"

AlternateLayout::AlternateLayout(lv_obj_t* parent) : LayoutBase(parent) {
    // 创建组件
...
void AlternateLayout::ArrangeWidgets() {
    ...
    if (clock_widget_) {
        ...
        lv_obj_set_size(clock_container, width * 0.8, height * 0.3);
        lv_obj_align(clock_container, LV_ALIGN_TOP_MID, 0, LayoutMetrics::MARGIN);
    }

    // 设置天气组件位置和大小
    if (weather_widget_) {
        lv_obj_t* weather_container = weather_widget_->GetContainer();
        lv_obj_set_size(weather_container, width * 0.8, height * 0.3);
        lv_obj_align(weather_container, LV_ALIGN_CENTER, 0, 0);
    }
